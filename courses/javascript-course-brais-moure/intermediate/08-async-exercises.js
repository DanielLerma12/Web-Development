...
function tiempo(ms) {
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve(console.log(`1 ya`));
    }, ms);
  });
}
async function process() {
  await Promise.all([tiempo(1000), tiempo(2000), tiempo(3000)]);
  console.log("Hey");
}
...
//     Retirando 300$...
//     Error: Fondos insuficientesfunction wait(ms) {
...
class Cajero {
  ...
  mensajeBienvenida() {
    return new Promise((resolve) => {
      setTimeout(() => {
        console.log(`Hola, ${this.nombre} bienvenido al banco Lermatraco!!!`);
        resolve();
      }, 1000);
    });
  }
  ...
  checkBalance() {
    return new Promise((resolve) => {
      setTimeout(() => {
        console.log(`Saldo actual del cajero: ${this.saldo}`);
        resolve();
      }, 1000);
    });
  }

  withdrawMoney(cantidad) {
    return new Promise((resolve, reject) => {
      console.log("Comprobando saldo...");
      setTimeout(() => {
        if (cantidad > this.saldo) {
          reject("Fondos insuficientes. Ingrese un valor más bajo.");
          return;
        }

        this.saldo -= cantidad;

        resolve(`Retiro exitoso por valor de ${cantidad}`);
      }, 3000);
    });
  }
}
...
async function main() {
  const persona1 = new Cajero("Daniel");

  await persona1.mensajeBienvenida();

  await persona1.checkBalance();

  const retiro = await persona1.withdrawMoney(1200000);
  console.log(retiro);
